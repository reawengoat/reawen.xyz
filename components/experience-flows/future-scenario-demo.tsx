...
import { ArrowRight, Check, Copy, Loader2, RotateCcw, Sparkles, TriangleAlert, Telescope } from "lucide-react";
...
const promptPresets = [
  "Bu hikaye umutlu mu, yoksa kırılgan mı?",
  "Sonuç kader gibi mi, yoksa seçim odaklı mı?",
  "Bu gelecek sinematik mi, yoksa sıradan mı?"
];

const defaultResult: GroqAnalysis = {
  headline: "Küçük ipuçları ver, geleceği kuralım",
  summary: "Kısa detaylarla olası bir gelecek sahnesi burada oluşur.",
  engagement: 0,
  balance: 0,
  tension: 0,
  insights: [
    { label: "İhtimal", value: "beklemede", tone: "Hazır" },
    { label: "Duygu tonu", value: "beklemede", tone: "Hazır" },
    { label: "Belirsizlik", value: "beklemede", tone: "Hazır" },
    { label: "Kısa not", value: "detayları girince sahne oluşur", tone: "Hazır" }
  ]
};

export function FutureScenarioDemo() {
  ...
  const shareSummary = hasResult
    ? [
        `Gelecek: ${result.headline}`,
        `Özet: ${result.summary}`,
        `İhtimal ${result.engagement}/100, denge ${result.balance}/100, belirsizlik ${result.tension}/100`,
        `Konu: ${subject || "anonim"}`,
        `Seçilen soru: ${selectedPreset}`
      ].join("\n")
    : "Önce konu ve ipucu ver, sonra sahneyi paylaş.";
      ...
      <div className="grid gap-6 lg:grid-cols-[1.02fr_0.98fr]">
        <Card className="bg-white/[0.05]">
          <CardHeader>
            ...
            <CardTitle className="text-3xl sm:text-4xl">AI Gelecek Senaryosu</CardTitle>
            <CardDescription className="text-base">
              Küçük ipuçları ver, kısa ve sinematik bir gelecek sahnesi oluştur.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form className="space-y-4" onSubmit={handleAnalyze}>
              ...
              <label className="block space-y-2">
                <span className="text-sm font-medium text-white/75">Konu</span>
                <input
                  value={subject}
                  onChange={(event) => setSubject(event.target.value)}
                  placeholder="Örn: kariyer, aşk, arkadaşlık, taşınma"
              ...
              <label className="block space-y-2">
                <span className="text-sm font-medium text-white/75">İpuçları</span>
                <textarea
                  value={message}
                  onChange={(event) => setMessage(event.target.value)}
                  placeholder="Örn: bir karar eşiğinde, birkaç ay sonra, küçük ama kritik bir değişim"
              ...
              <label className="block space-y-2">
                <span className="text-sm font-medium text-white/75">Duygu tonu</span>
                <input
                  value={context}
                  onChange={(event) => setContext(event.target.value)}
                  placeholder="Örn: umutlu ama biraz belirsiz"
                  className="h-12 w-full rounded-[1.25rem] border border-white/10 bg-slate-950/55 px-4 text-sm text-white outline-none transition focus:border-white/20"
                />
              </label>
              ...
              <Button type="submit" className="w-full" disabled={isLoading}>
                ...
                {isLoading ? "Groq analiz ediyor" : "Gelecek sahnesini üret"}
              </Button>
            </form>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white/[0.05]">
          <CardHeader>
            ...
            <CardDescription className="text-base">Sahne üretildikçe kart canlı olarak değişir.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {[['İhtimal', result.engagement], ['Denge', result.balance], ['Belirsizlik', result.tension]].map(([label, value]) => (
            ...
            <div className="rounded-[1.5rem] border border-white/8 bg-slate-950/55 p-5">
              <div className="flex items-center gap-2 text-sm text-white/65">
                <Telescope className="h-4 w-4 text-white/45" />
                {selectedPreset}
              </div>
